import { HandlerEvent } from "@netlify/functions";
import { describe, expect, it } from "vitest";
import { z } from "zod";

import { parseBody } from "../parseBody";
import { createRouterResponse, isRouterResponse } from "../router";

function makeEvent(body: string | null): HandlerEvent {
  return {
    path: "/",
    httpMethod: "POST",
    body,
    headers: {},
    multiValueHeaders: {},
    queryStringParameters: null,
    multiValueQueryStringParameters: null,
    isBase64Encoded: false,
    rawUrl: "http://localhost/",
    rawQuery: "",
  };
}

const schema = z.object({ name: z.string() });

describe("parseBody", () => {
  it("returns the parsed data for a valid body", () => {
    const result = parseBody(
      makeEvent(JSON.stringify({ name: "club" })),
      schema,
      createRouterResponse,
    );
    expect(isRouterResponse(result)).toBe(false);
    expect(result).toEqual({ name: "club" });
  });

  it("returns a 400 when the body is missing", () => {
    const result = parseBody(makeEvent(null), schema, createRouterResponse);
    expect(isRouterResponse(result)).toBe(true);
    if (!isRouterResponse(result)) throw new Error("expected a RouterResponse");
    expect(result.response.statusCode).toBe(400);
  });

  it("returns a 400 when the body is empty", () => {
    const result = parseBody(makeEvent(""), schema, createRouterResponse);
    expect(isRouterResponse(result)).toBe(true);
    if (!isRouterResponse(result)) throw new Error("expected a RouterResponse");
    expect(result.response.statusCode).toBe(400);
  });

  it("returns a 400 instead of throwing when the body is malformed JSON", () => {
    const result = parseBody(makeEvent("{not json"), schema, createRouterResponse);
    expect(isRouterResponse(result)).toBe(true);
    if (!isRouterResponse(result)) throw new Error("expected a RouterResponse");
    expect(result.response.statusCode).toBe(400);
  });

  it("returns a 400 when the body fails schema validation", () => {
    const result = parseBody(makeEvent(JSON.stringify({ name: 5 })), schema, createRouterResponse);
    expect(isRouterResponse(result)).toBe(true);
    if (!isRouterResponse(result)) throw new Error("expected a RouterResponse");
    expect(result.response.statusCode).toBe(400);
  });

  it("applies schema defaults to the parsed output", () => {
    const withDefault = z.object({ type: z.enum(["a", "b"]).default("a") });
    const result = parseBody(makeEvent(JSON.stringify({})), withDefault, createRouterResponse);
    expect(result).toEqual({ type: "a" });
  });
});
