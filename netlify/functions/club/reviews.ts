import { ReviewResponse, DetailedReviewResponse, Club, DateObject } from "@/models";
...
const modifyPath = new Path<StringRecord>('/api/club/:clubId<\\d+>/reviews/:movieId<\\d+>')
...
export async function handler(event: HandlerEvent, context: HandlerContext, _path: StringRecord): Promise<HandlerResponse> {
  ...
  switch (event.httpMethod) {
    ...
    case "PUT": {
      ...
      if (!body.name || !body.score) return badRequest('Missing required body parameters');

      return await updateReviewScore(clubId, parseInt(path.movieId), body.name, body.score)
    }
    default:
...
async function getReviews(clubId: number, detailed: boolean): Promise<HandlerResponse> {
  ...
  if (detailed) {
    const detailedReviews = await getDetailedMovieData(reviews.reviews as DetailedReviewResponse[])
