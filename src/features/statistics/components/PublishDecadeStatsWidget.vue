<template>
  <WidgetShell
    v-if="decadeStats.length > 0"
    title="Scores by Publication Decade"
  >
...
import { computePublishDecadeStats } from "../statsComputers";
...
const decadeStats = computed(() =>
  computePublishDecadeStats(props.bookData, selectedMemberId.value),
);

const chartOptions = computed(() =>
  createDecadeChartOptions(decadeStats.value, "Books"),
);
</script>
