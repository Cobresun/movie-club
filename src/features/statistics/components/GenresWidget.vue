    ...
    <template v-if="mode === 'rated'">
      ...
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div v-if="genreStats.mostLoved.length > 0" class="text-left">
          <h4 class="mb-3 text-sm font-semibold text-emerald-400">Most Loved</h4>
          <ul class="space-y-3">
            <li
    ...
    <template v-else>
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div v-if="watchCounts.mostWatched.length > 0" class="text-left">
          <h4 class="mb-3 text-sm font-semibold text-emerald-400">Most Watched</h4>
          <ul class="space-y-3">
            <li
        ...
        <div v-if="watchCounts.leastWatched.length > 0" class="text-left">
          <h4 class="mb-3 text-sm font-semibold text-rose-400">Least Watched</h4>
          <ul class="space-y-3">
            <li
...
const genreStats = computed(() => computeGenreStats(props.movieData, selectedMemberId.value));
...
const maxCount = computed(() =>
  watchCounts.value.mostWatched.length > 0 ? watchCounts.value.mostWatched[0].count : 1,
);
