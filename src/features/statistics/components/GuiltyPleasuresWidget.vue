...
import { computeGuiltyPleasures } from "../statsComputers.js";
...
const selectedMemberId = ref<string | undefined>(
  guiltyPleasures.value[0]?.member.id,
);

const selectedEntry = computed(() =>
  guiltyPleasures.value.find((e) => e.member.id === selectedMemberId.value),
);

watch(guiltyPleasures, (entries) => {
  if (
