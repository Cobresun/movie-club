...
- **`GalleryView`** renders what `ReviewView` hands it — the search-filtered reviews, and the club's members the sort menu is built from. Render `ReviewView` rather than handing the gallery a list of your own: a hand-picked one sorts perfectly while the wiring between the two is wrong.
