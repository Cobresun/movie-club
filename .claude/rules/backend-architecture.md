...
A score gesture writes to the works it _resolves_ to, not to the work it names: `MediaProvider.expandScoreTargets(work, { seasonNumber, episodeNumber })` returns the further works a score fans out to — `undefined` for a movie, a book or one TV episode (scored directly), one entry per episode for a TV season or show, and exactly one for a show narrowed to an episode that is not a work yet. An empty array means the gesture resolved to nothing and is refused. `POST /reviews` upserts those works, puts them on the reviews list and writes one review each, so there is no bulk-score endpoint and no club-type branch in the handler. Adding a second such endpoint is the mistake this design exists to prevent.
